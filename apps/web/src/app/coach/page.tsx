...
function EmptyState({ icon, text, cta }: { icon: React.ReactNode; text: string; cta?: React.ReactNode }) {
  return (
    <div className="card p-8 text-center space-y-3">
      <div className="w-12 h-12 rounded-lg bg-pitch-soft text-pitch flex items-center justify-center mx-auto">{icon}</div>
  ...
  return (
    <div className="space-y-8">
      <section className="space-y-3">
        <h2 className="display text-lg text-ink px-1">Mes matchs</h2>
        {matches.length === 0 && (
          <EmptyState
                ...
                <MatchCard key={match.id} match={match}>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-ink-soft font-semibold flex items-center gap-1.5">
                      <Car size={13} className="text-tangerine" /> Covoiturage : {match.transportSeats} place{match.transportSeats > 1 ? "s" : ""}
                    </span>
      ...
      <section className="space-y-3">
        <h2 className="display text-lg text-ink px-1">Mes annonces</h2>
          ...
          <div key={a.id} className="card p-4 flex items-center justify-between gap-3 animate-rise-in">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className={a.status === "open" ? "chip bg-sky-soft text-sky" : "chip bg-pitch-soft text-pitch-deep"}>
                {a.status === "open" ? "Ouverte" : "Adversaire trouvé"}
              </span>
              {a.status === "open" && (
                <button
