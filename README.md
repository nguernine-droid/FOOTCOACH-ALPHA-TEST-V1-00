...
- L'onglet **Annonces** range tout ce qui se publie en **trois catégories** : *Amicaux* et *Tournois* (ce que les autres cherchent, la matière du radar sans la carte) et ***Les miennes*** — ce que j'ai publié, avec ses trois casiers (en cours / confirmées / passées) et les propositions à trancher. Cette troisième catégorie et la page « Moi › Mes annonces » sont **le même composant** (`components/announcements/MyPublications.tsx`, hook `useMyPublications` + `MyPublicationsList`) : c'est ici qu'on vient quand on pense « annonces », et revenir sur la sienne ne doit pas demander de changer d'écran. Son compteur ne montre que ce qui n'est **pas encore passé** — un onglet doit dire ce qui vit, pas la taille des archives.
