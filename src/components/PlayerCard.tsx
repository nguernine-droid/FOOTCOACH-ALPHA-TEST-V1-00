...
export function PlayerCard({ player, onClick, isSelectionMode, isSelected, onToggleSelect }: PlayerCardProps) {
  return (
    <div
      ...
      className={`bg-black rounded-[2.5rem] relative overflow-hidden shadow-[0_0_20px_rgba(0,0,0,0.8)] transition-all cursor-pointer group border-2 ${
        isSelected ? 'border-neon-cyan ring-4 ring-neon-cyan/20' : 'border-neon-cyan/30 hover:border-neon-cyan/60'
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-neon-cyan/40 rounded-tl-[2.5rem]" />
      <div className="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-neon-cyan/40 rounded-br-[2.5rem]" />

      {isSelectionMode && (
        <div className={`absolute top-4 right-4 z-20 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-neon-cyan border-neon-cyan shadow-[0_0_10px_#00F0FF]' : 'bg-black/50 border-white/30'}`}>
      ...
      <div className="p-5 pt-8 flex flex-col items-center flex-1 relative z-10">
        ...
        <div className="w-full flex justify-between items-start mb-4">
          <div className="text-left font-mono">
            <p className="text-4xl font-black text-white leading-none tracking-tighter drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{player.rating}</p>
            <p className="text-[9px] font-black text-neon-cyan uppercase italic tracking-widest mt-1">{player.position}</p>
          </div>
        ...
        <div className="relative mb-4 group-hover:scale-105 transition-transform duration-500">
           <div className="absolute inset-0 bg-neon-cyan/20 blur-xl opacity-0 group-hover:opacity-40 transition-opacity" />
           <div className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-neon-cyan/20 shadow-2xl relative bg-gray-900">
              <img
        ...
        {/* Name */}
        <h4 className="font-black text-sm text-white text-center uppercase italic tracking-tight mb-5 px-1 leading-tight line-clamp-1 group-hover:text-neon-cyan transition-colors">
          {player.name}
        </h4>

        {/* Mini Stats Grid - NEON BARS STYLE */}
        <div className="w-full space-y-3 bg-white/[0.03] border border-white/5 rounded-2xl p-4 font-mono">
          ...
          <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
             <div className="h-full bg-neon-cyan shadow-[0_0_8px_#00F0FF]" style={{ width: `${player.rating}%` }} />
          </div>
