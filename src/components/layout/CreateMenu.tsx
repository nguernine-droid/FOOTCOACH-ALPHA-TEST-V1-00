...
export function CreateMenu({ isOpen, onClose, context }: CreateMenuProps) {
  ...
  const getMenuItems = () => {
    ...
    // AJOUT SYSTÉMATIQUE DU BOUTON BUG EN VERSION TEST (En haut de la liste)
    const feedbackItems = [
      {
        label: 'Signaler Bug',
        desc: 'Un problème ? Aide-nous à corriger !',
        color: 'bg-red-600',
        icon: <Bug size={26} />,
        href: '/settings'
      },
    ...
    // On fusionne les feedbacks en version Alpha (toujours visible)
    return [...feedbackItems, ...items];
  };
      ...
      <div className="relative w-full max-w-md bg-dark-card rounded-[3rem] p-8 shadow-2xl border border-white/5 animate-in slide-in-from-bottom duration-300">
        <div className="flex justify-between items-center mb-10 px-2 text-left">
          <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">
            {context === 'radar' ? 'Matchmaking' : context === 'team' ? 'Effectif' : context === 'com' ? 'Briefing' : context === 'calendar' ? 'Planning' : 'Nexus_OS'}
          </h2>
        ...
        <div className="space-y-3 mb-4 max-h-[60vh] overflow-y-auto no-scrollbar pr-1">
          {menuItems.map((item, i) => (
            <Link
              ...
              href={item.href}
              onClick={onClose}
              className={`${item.color} w-full rounded-[2rem] p-5 flex items-center gap-5 text-white shadow-lg active:scale-[0.98] transition-all text-left group border border-white/10`}
            >
              <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-md border border-white/20 group-hover:scale-110 transition-transform flex-shrink-0">
                {item.icon}
              </div>
              <div className="flex-1 min-w-0">
                <span className="font-black text-lg leading-none uppercase italic block mb-1 tracking-tight">{item.label}</span>
                <span className="text-[9px] font-bold text-white/70 uppercase tracking-widest block truncate">{item.desc}</span>
              </div>
