...
export default function NewSignalPage() {
  ...
  const styles = isPro ? {
    mainBg: 'bg-gray-50', cardBg: 'bg-white border-gray-100 shadow-sm', text: 'text-gray-900', textSub: 'text-gray-400', accent: 'text-orange-600', accentBg: 'bg-orange-600 text-white', inputBg: 'bg-gray-100 border-gray-100'
  } : {
    mainBg: 'bg-black', cardBg: 'bg-white/5 border-white/10', text: 'text-white', textSub: 'text-gray-500', accent: 'text-neon-cyan', accentBg: 'bg-neon-cyan text-black shadow-[0_0_20px_#00F0FF]', inputBg: 'bg-white/5 border-white/10'
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!category) { alert("Veuillez renseigner la catégorie."); return; }
    setIsLoading(true);
    ...
    try {
      ...
      const { error } = await supabase.from('match_requests').insert([{
        coach_id: user.id, type, category, date, time,
      ...
      alert("✅ SIGNAL_ÉMIS !");
      router.push('/radar');
    } catch (err: any) { alert("Erreur : " + err.message); } finally { setIsLoading(false); }
  };
  ...
  return (
    <main className={`min-h-screen max-w-md mx-auto shadow-2xl pb-20 font-sans ${styles.mainBg}`}>
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-6 text-left">
        {/* CATÉGORIE */}
        <section className="space-y-3">
           ...
           <div className="grid grid-cols-3 gap-2">
              {categories.map(cat => (
                <button key={cat} type="button" onClick={() => setCategory(cat)} className={`py-2 rounded-xl text-[9px] font-black border transition-all ${category === cat ? (isPro ? 'bg-orange-600 text-white border-orange-600 shadow-md' : 'bg-neon-cyan text-black border-neon-cyan shadow-[0_0_10px_#00F0FF66]') : (isPro ? 'bg-white border-gray-200 text-gray-400' : 'bg-white/5 border-white/5 text-gray-600')}`}>{cat}</button>
              ))}
           </div>
        </section>
        ...
        <section className="space-y-3">
          ...
          <div className={`${styles.cardBg} rounded-2xl p-1 border flex gap-1`}>
             ...
             <ToggleButton active={travelPreference === 'away'} onClick={() => setTravelPreference('away')} label="Déplacement" icon={<Navigation size={16} />} isPro={isPro} styles={styles} />
        ...
        {/* DATE & HEURE */}
        <section className={`${styles.cardBg} rounded-2xl p-6 border grid grid-cols-2 gap-4`}>
            <div className="space-y-2">
              <label className={`text-[9px] font-bold uppercase flex items-center gap-1.5 ${styles.textSub}`}><Calendar size={12}/> Date</label>
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} required className={`w-full ${styles.inputBg} rounded-xl p-3 text-xs font-black outline-none border ${isPro ? 'text-gray-900 border-gray-100' : 'text-white border-white/5'}`} />
            </div>
            <div className="space-y-2">
              <label className={`text-[9px] font-bold uppercase flex items-center gap-1.5 ${styles.textSub}`}><Clock size={12}/> Heure</label>
              <input type="time" value={time} onChange={(e) => setTime(e.target.value)} required className={`w-full ${styles.inputBg} rounded-xl p-3 text-xs font-black outline-none border ${isPro ? 'text-gray-900 border-gray-100' : 'text-white border-white/5'}`} />
            </div>
        </section>
