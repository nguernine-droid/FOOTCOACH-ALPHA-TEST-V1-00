    ...
    <div className={`min-h-screen overflow-x-hidden relative ${isPro ? 'bg-gray-50' : 'bg-black text-white'}`}>
      {showSuccess && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="relative bg-white rounded-[3rem] p-10 flex flex-col items-center text-center shadow-2xl animate-in zoom-in duration-500 max-w-xs">
            ...
            <h2 className="text-2xl font-black text-gray-900 uppercase italic tracking-tighter mb-2 leading-none">Transmission Réussie</h2>
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest leading-relaxed">Événement ajouté à l'unité.</p>
          </div>
        </div>
      )}
      ...
      <main className="max-w-md mx-auto min-h-screen relative pb-80 shadow-2xl font-sans text-left">
        <header className={`py-5 px-6 sticky top-0 z-40 border-b flex items-center gap-4 backdrop-blur-md ${isPro ? 'bg-white/95 border-gray-100' : 'bg-black/80 border-white/10'}`}>
          <button onClick={() => router.back()} className={`active:scale-90 transition-transform p-1 -ml-1 rounded-full ${isPro ? 'text-gray-900' : 'text-white'}`}>
            <ChevronLeft size={28} strokeWidth={3} />
          </button>
          <h1 className={`text-2xl font-black uppercase italic tracking-tighter ${isPro ? 'text-gray-900' : 'text-white'}`}>Planifier_Mission</h1>
        </header>

        <div className="p-6 space-y-8">
          {/* SLIDE SELECTOR */}
          <section className="space-y-4">
            <h3 className={`px-2 text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 ${isPro ? 'text-gray-400' : 'text-gray-500'}`}>Type d'événement</h3>
            <div
              ref={scrollRef}
              onScroll={handleScroll}
              className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar rounded-[2.5rem] shadow-xl border border-white/5"
            >
              {eventTypes.map((t) => (
                <div
                  key={t.id}
          ...
          <div className="space-y-6">
            ...
            <div className={`${isPro ? 'bg-white border-gray-100' : 'bg-white/5 border-white/10'} rounded-[2.5rem] p-7 shadow-sm border space-y-5`}>
              ...
              {type === 'training' && (
                <div className="space-y-3">
                  <label className={`text-[10px] font-black uppercase tracking-widest block ml-2 ${isPro ? 'text-gray-400' : 'text-gray-500'}`}>Objectif Technique</label>
                  <div className="flex flex-wrap gap-2">
                    {trainingThemes.map(themeItem => (
                      <button
                        key={themeItem.id}
                        type="button"
                        onClick={() => setTrainingTheme(themeItem.id)}
                        className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase flex items-center gap-2 transition-all border ${
                          trainingTheme === themeItem.id
                            ...
                            : `${isPro ? 'bg-gray-50 border-gray-100 text-gray-400' : 'bg-white/5 border-white/5 text-gray-600'}`
                        }`}
                      >
                        {themeItem.icon}
                        {themeItem.id}
            ...
            <div className={`${isPro ? 'bg-white border-gray-100' : 'bg-white/5 border-white/10'} rounded-[2.5rem] p-7 shadow-sm border grid grid-cols-2 gap-4`}>
                <div className="space-y-3">
                  <label className={`text-[10px] font-black uppercase block ml-2 flex items-center gap-2 ${isPro ? 'text-gray-400' : 'text-gray-500'}`}><Calendar size={14} className="text-brand-orange" /> Date</label>
                ...
                <div className="space-y-3">
                  <label className={`text-[10px] font-black uppercase block ml-2 flex items-center gap-2 ${isPro ? 'text-gray-400' : 'text-gray-500'}`}><Clock size={14} className="text-brand-orange" /> Heure</label>
            ...
            <div className={`${isPro ? 'bg-white border-gray-100' : 'bg-white/5 border-white/10'} rounded-[2.5rem] p-7 shadow-sm border space-y-3`}>
              <label className={`text-[10px] font-black uppercase block ml-2 flex items-center gap-2 ${isPro ? 'text-gray-400' : 'text-gray-500'}`}><MapPin size={14} className="text-brand-orange" /> Localisation</label>
            ...
            {/* Notes */}
            <div className={`${isPro ? 'bg-white border-gray-100' : 'bg-white/5 border-white/10'} rounded-[2.5rem] p-7 shadow-sm border space-y-3 text-left`}>
              <label className={`text-[10px] font-black uppercase block ml-2 ${isPro ? 'text-gray-400' : 'text-gray-500'}`}>Détails / Consignes</label>
              <textarea placeholder="Matériel requis, objectifs..." value={note} onChange={(e) => setNote(e.target.value)} className={`w-full ${isPro ? 'bg-gray-50 text-gray-900' : 'bg-black/40 text-white'} rounded-2xl p-5 text-sm font-medium outline-none min-h-[140px] resize-none border border-transparent focus:border-brand-orange/20`} />
            </div>
          </div>
        </div>
        ...
        <div className="fixed bottom-28 left-0 right-0 z-40 px-6 py-4 pointer-events-none">
          <div className="max-w-md mx-auto w-full pointer-events-auto">
            <button
              onClick={handleSave}
              className={`w-full ${isPro ? 'bg-orange-600 text-white' : 'bg-neon-cyan text-black shadow-[0_0_20px_#00F0FF]'} font-black py-6 rounded-[2.5rem] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-4 uppercase italic tracking-tighter text-xl`}
            >
